...
function handleError(res, reason, messageForUser, code) {
  ...
  res
    ...
    .send({
      success: false,
      message: messageForUser || "An unknown error occurred.",
    });
}
      ...
      if (err) {
        ...
        if (Array.isArray(docs)) {
          ...
          handleError(
            ...
            "Failed to get items. Returned but was not an array."
          );
          return;
        }
      }
...
app.post("/api/reserve/:id", (req, res) => {
  ...
  if (!data) {
    handleError(
      res,
      "Data missing in request for reservation",
      "Data missing in request.",
      400
    );
    return;
  }
  ...
  if (!data.email || typeof data.email !== "string") {
    handleError(
      res,
      "User did not supply email for reservation",
      "You must supply your email to reserve a gift.",
      400
    );
    return;
  }
  ...
  if (!data.name || typeof data.name !== "string") {
    handleError(
      res,
      "User did not supply name for reservation",
      "You must supply your name(s) to reserve a gift.",
      400
    );
    return;
  }
  ...
  if (!req.params.id) {
    handleError(
      res,
      "ID missing in reservation",
      "ID was missing in request.",
      400
    );
    return;
  }
      ...
      function(err, doc) {
        ...
        if (err) {
          ...
          if (!doc) {
            handleError(
              res,
              "Document could not be found for reservation",
          ...
          if (!doc.available) {
            handleError(
              res,
              "Document was not available for reservation",
            ...
            function(err, result) {
              ...
              if (err) {
                ...
                if (result.modifiedCount > 0) {
                  ...
                  handleError(
                    res,
                    "No items modified in reservation request.",
...
app.post("/api/cancel-reservation/:id", (req, res) => {
  ...
  if (!data) {
    handleError(
      res,
      "Data missing in request for cancellation",
      "Data missing in request.",
      400
    );
    return;
  ...
  if (!data.email || typeof data.email !== "string") {
    handleError(
      res,
      "User did not supply email for cancellation",
      "You must supply your email to cancel a reservation.",
      400
    );
    return;
  }
  ...
  console.log(
    data.email + " is attempting to cancel reservation: " + req.params.id
  );
      ...
      function(err, doc) {
        ...
        if (err) {
          handleError(
            res,
            err.message,
            "Failed to cancel reservation for item."
          );
          return;
        } else {
          if (!doc) {
            handleError(
              res,
              "Document could not be found to cancel the reservation",
              "This item could not be found for to cancel the reservation.",
              400
            );
            return;
          }
          if (doc.available === true) {
            handleError(
              res,
              "Document was available during reservation cancellation",
              "This item is already available for reservation.",
              400
            );
            return;
          }
          if (doc.email !== data.email) {
            ...
            if (data.email === "masterpepee@queen.com") {
              console.log(
                "Master email address has been used to cancel reservation for: " +
                  doc._id +
                  " " +
                  doc.email +
                  " " +
              ...
              handleError(
                res,
                "User email did not match item email: doc: " +
                  doc.email +
                  " data: " +
                  data.email,
                "This email does not match the one used to make the reservation for this item.",
                400
              );
              return;
            }
                ...
                if (err) {
                  ...
                  if (result.modifiedCount > 0) {
                    res.status(200).send({
                      success: true,
                      message:
                        "You have successfully cancelled your reservation to give this gift.",
                    });
                  } else {
                    handleError(
                      res,
                      "No items modified in cancel reservation request.",
                      "No changes could be made."
                    );
                    return;
                  }
